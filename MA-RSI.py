...
st.title("Analyse des variations de prix avec moyenne mobile et RSI")
...
if data is not None:
    ...
    rsi_period = st.slider("Période du RSI", 5, 50, 14)
    rsi_overbought = st.slider("RSI - Seuil de surachat", 50, 100, 70)
    rsi_oversold = st.slider("RSI - Seuil de survente", 0, 50, 30)
    ...
    # Appliquer la condition sur le RSI
    data["RSI_Condition"] = data["RSI"] > rsi_overbought

    # Calculer les variations maximales
    variations = []
    for i in range(len(data) - future_days):
        if data["Signal"].iloc[i] == 1 and data["RSI_Condition"].iloc[i]:
